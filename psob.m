%POSB.m
function [location,fbest]=psob(choose,value,M,D)%choose：求上层还是下层，value：初始值，M：最大迭代次数，D:问题的维度
N=40;
% c1=20;c2=50;c3=30;
c1=0.5;c2=2;c3=3;
w=0.9;
...
%------天牛须部分的参数---------
        dir(i,:)=rands(1,D);%头在k维空间的朝向是随机的
        dir(i,:)=dir(i,:)/(eps+norm(dir(i,:)));%方向向量归一化,方向
        xleft(i,:)=x(i,:)+dir(i,:)*d0/2;%空间中左须位置随机
        fleft(i,:)=fitness(xleft(i,:));%左须位置的函数值
        xright(i,:)=x(i,:)-dir(i,:)*d0/2;%右须位置
        fright(i,:)=fitness(xright(i,:));%右须位置函数值
        force_bas(i,:) = -dir(i,:)*sign(fleft(i,:)-fright(i,:));
        
        
        v(i,:)=w*v(i,:)+c1*rand*(y(i,:)-x(i,:))+c2*rand*(pg-x(i,:))+c3*rand*force_bas(i,:); 
        x(i,:)=x(i,:)+v(i,:);
        if fitness(x(i,:))<p(i)%更新个体最佳  
            p(i)=fitness(x(i,:));  
            y(i,:)=x(i,:);  
        end  
        if p(i)<fitness(pg)%更新全局最佳  
            pg=y(i,:);  
        end  
%           v(i,:)=w*v(i,:)+c1*rand*(y(i,:)-x(i,:))+c2*rand*(pg-x(i,:))+c3*rand*force_bas(i,:); 
%           x(i,:)=x(i,:)+v(i,:);
